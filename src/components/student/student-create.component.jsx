...
class StudentCreate extends Component {
  ...
  handleShareholderNameChange = (idx) => (evt) => {
    const newShareholders = this.state.studentDetails.associatedParentList.map(
      (shareholder, sidx) => {
  ...
  changeHandler = (event) => {
    ...
    if (name.includes("associatedParentList")) {
      const test = name.split(".");
      console.log("test", test[1]);
      let prentItems = item.associatedParentList[0];
      prentItems[test[1]] = value;
    } else {
      item[name] = value;
    }

    console.log("item", item);
    console.log("item", item.associatedParentList);
  ...
  handelSubmit = (event) => {
    ...
    alert("handelSubmit", studentDetails);
